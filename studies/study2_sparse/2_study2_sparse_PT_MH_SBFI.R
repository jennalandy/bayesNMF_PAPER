library(bayesNMF)
library(glue)
job_assignments <- read.csv("study2_sparse_assignments.csv")
...
output_dir <- glue("../../output/study2_sparse/{L}_{P}{ifelse(MH, '_MH', '')}_{rank_method}")
print(output_dir)
########################################################
