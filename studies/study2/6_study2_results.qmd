...
plot_dat <- results %>%
  mutate(
    ...
    method = case_when(
      grepl('signatureanalyzer', model) ~ "SignatureAnalyzer",
      grepl('SBFI', model) ~ "bayesNMF, SBFI",
      grepl('BIC', model) ~ "bayesNMF, Heuristic",
      TRUE ~ "none"
    ),
...
names = list(
  ...
  'P_E_MH_BIC' = "bayesNMF Heuristic",
  'P_T_MH_BIC' = "bayesNMF Heuristic",
  'signatureanalyzer_L1' = "SignatureAnalyzer",
  'signatureanalyzer_L2' = "SignatureAnalyzer"
)

legend_plot <- expand.grid(
  method = c("bayesNMF SBFI", "bayesNMF Heuristic", "SignatureAnalyzer"),
...
greyscale_plot <- function(levels, med_size = 1, legend_height = 0.1, no_x_title = FALSE, no_legend = FALSE) {
...
ggsave(file.path(figures_dir, "rank_metrics_aligned_greyscale_1_20.png"), pl, width = 14, height = 4)

pl <- greyscale_plot(1:10, no_x_title = TRUE)
ggsave(file.path(figures_dir, "rank_metrics_aligned_greyscale_1_10.png"), pl, width = 7, height = 4)
...
ggsave(file.path(figures_dir, "rank_metrics_aligned_greyscale_5s.png"), pl, width = 7, height = 5)
```
