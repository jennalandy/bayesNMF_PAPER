...
processed_dir <- "../../processed/study1_save"
...
ggsave(file.path(figures_dir, "study1_metrics.png"), width = 10, height = 10)
```
...
```{r}
Poisson_vs_Normal_MAPs <- read_csv(file.path(processed_dir, "Poisson_vs_Normal_MAPs.csv"))
...
Poisson_vs_Normal_MAPs %>%
  arrange(P_min_cosine) %>%
  select(N, G, rep, P_min_cosine, E_min_cosine)

mean(Poisson_vs_Normal_MAPs$P_min_cosine > 0.99)
mean(Poisson_vs_Normal_MAPs$E_min_cosine > 0.99)
...
mean(Standard_vs_MH_MAPs$P_min_cosine > 0.99)
mean(Standard_vs_MH_MAPs$E_min_cosine > 0.99)


Standard_vs_Standard_repeated_MAPs %>%
  arrange(P_min_cosine) %>%
  select(N, G, rep, P_min_cosine, E_min_cosine)

mean(Standard_vs_Standard_repeated_MAPs$P_min_cosine > 0.99)
mean(Standard_vs_Standard_repeated_MAPs$E_min_cosine > 0.99)

metrics %>%
  filter(MH, likelihood == "P", N == 16) %>%
  pull(total_minutes) %>%
  summary()
...
fig1A <- Poisson_vs_Normal_MAPs %>%
  ...
  ggplot(aes(x = value, y = prior, fill = prior, color = prior, alpha = prior)) +
  facet_grid(cols = vars(name), rows = vars(compare), switch = 'y') +
  geom_vline(xintercept = 0.99, linetype = "dashed") +
  geom_density_ridges(scale = 2, bandwidth = 0.001, linewidth = 1) +
  labs(
    x = "Minimum cosine similarity between MAP estimates",
    fill = "Prior", color = "Prior", alpha = "Prior"
  ) +
  ...
  scale_alpha_manual(values = c("Exponential" = 0.5, "Gamma / Truncated Normal" = 0))
```
...
fig1B <- dat %>%
  ...
  ggplot(aes(x = as.factor(G), y = speedup, color = prior, fill = prior, alpha = prior)) +
...
fig1C <- metrics %>%
  ...
  ggplot(aes(x = as.factor(G), y = value, color = prior, fill = prior, alpha = prior)) +
...
legend <- g_legend(fig1A)
blank <- ggplot() + theme_void()
margin <- 0.02

fig1A_with_caption <- ggdraw(
  fig1A + 
...
rowAB <- plot_grid(
  ...
  fig1B + 
    theme(legend.position = "none") + 
    ggtitle("B. Relative Efficiency\n     of Poisson + MH relative to Poisson") +
    theme(
      plot.title.position = "plot",
      axis.title.y = element_blank()
    ),
...
ggsave(file.path(figures_dir, "study1.png"), fig1, width = 20, height = 20)
```

## Figure 2: comparing credible intervals
...
fig_Es <- Standard_vs_MH_CIs %>%  
  ...
  geom_hline(yintercept = 0, linetype = "dashed") +
  labs(
    y = "Median Difference in CI width: Poisson - (Poisson + MH)\nScale = Proportion of Sample-Wise Mutations",
