...
Poisson_vs_Normal_MAPs_Exponential <- process_compare_MAPs("exponential")
Poisson_vs_Normal_MAPs_Gamma <- process_compare_MAPs("gamma")
Poisson_vs_Normal_MAPs <- Poisson_vs_Normal_MAPs_Exponential %>%
  mutate(prior = 'Exponential') %>%
  rbind(
    Poisson_vs_Normal_MAPs_Gamma %>%
    mutate(prior = "Gamma / Truncated Normal")
  )
write_csv(Poisson_vs_Normal_MAPs, file.path(processed_dir, "Poisson_vs_Normal_MAPs.csv"))

Standard_vs_MH_MAPs_Exponential <- process_compare_MAPs("exponential", c("Poisson", "Poisson_MH"))  
Standard_vs_MH_MAPs_Gamma <- process_compare_MAPs("gamma", c("Poisson", "Poisson_MH"))
Standard_vs_MH_MAPs <- Standard_vs_MH_MAPs_Exponential %>%
  mutate(prior = 'Exponential') %>%
  rbind(
    Standard_vs_MH_MAPs_Gamma %>%
    mutate(prior = "Gamma / Truncated Normal")
  )
...
Standard_vs_Standard_repeated_MAPs_Gamma <- process_compare_MAPs("gamma", c("Poisson", "Poisson_repeated"))
Standard_vs_Standard_repeated_MAPs <- Standard_vs_Standard_repeated_MAPs_Exponential %>%
  mutate(prior = 'Exponential') %>%
  rbind(
    Standard_vs_Standard_repeated_MAPs_Gamma %>%
    mutate(prior = "Gamma / Truncated Normal")
  )
