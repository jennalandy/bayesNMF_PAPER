...
Note: Rownames of PCAWG data overlap but are not in the same order as the rownames of the COSMIC reference. However, bayesNMF auto-aligns rownames of reference to the rownames of the data (in assign_signatures_ensemble used for bayesNMF methods, and in hungarian_assignment used for SignatureAnalyzer). So we don't need to manually align the reference to the data.

```{r}
setequal(rownames(reference), rownames(read.csv(file.path(processed_dir, "matrices_nonhyper", "Biliary_AdenoCA.csv"), row.names = 1)))
all(rownames(reference) == rownames(read.csv(file.path(processed_dir, "matrices_nonhyper", "Biliary_AdenoCA.csv"), row.names = 1)))
```
...
results_df <- do.call(rbind, lapply(results_list, function(res) {
  ...
  assigned <- bayesNMF::hungarian_assignment(Phat, reference)
...
assignments_greyscale <- combo_df %>%
  ...
  mutate(
    ...
    Signature = factor(Signature, levels = included_signatures)
  ) %>%
...
assignments_greyscale_with_caption <- assignments_greyscale + 
    ggtitle("A. Comparison to COSMIC v3.3.1 SBS Reference Signatures") +
    theme(
...
fig_greyscale <- gridExtra::arrangeGrob(
  gridExtra::arrangeGrob(g1, g2, nrow = 1, widths = c(5, 1.2)),
  legend_greyscale,
  nrow = 2,
  heights = c(10, 1)
)
